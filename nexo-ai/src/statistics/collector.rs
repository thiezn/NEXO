...
use super::backend::{InMemoryBackend, StatsBackend};
use super::metrics::{InferenceDetail, InferenceRecord, LifecycleEvent, LifecycleRecord};
...
impl StatsCollector {
    ...
    pub fn log_memory_pressure(&self, model_name: &str) {
        let free = crate::inference::candle::device::free_system_memory_bytes();
        let available = crate::inference::candle::device::available_system_memory_bytes();
        tracing::info!(
            model = model_name,
            free_mb = free.map(|bytes| bytes / (1024 * 1024)),
            available_mb = available.map(|bytes| bytes / (1024 * 1024)),
            "memory pressure"
        );
    }
    ...
    pub fn model_stats(&self, name: &str, category: ModelCategory) -> Option<&ModelStats> {
        self.backend.model_stats(name, category)
    }
...
mod tests {
    ...
    fn record_and_query_transcription() {
        ...
        let model_stats = stats.model_stats("whisper", ModelCategory::Listen).unwrap();
        assert_eq!(model_stats.inference_count, 1);
        assert!((model_stats.primary_metric.mean() - 0.4).abs() < 0.001);
    }
    ...
    fn record_and_query_synthesis() {
        ...
        let model_stats = stats.model_stats("tts-model", ModelCategory::Talk).unwrap();
        assert!((model_stats.primary_metric.mean() - 2.0).abs() < 0.001);
    }
    ...
    fn record_and_query_image_generation() {
        ...
        let model_stats = stats.model_stats("flux", ModelCategory::Imagine).unwrap();
        assert!((model_stats.primary_metric.mean() - 0.25).abs() < 0.001);
    }
