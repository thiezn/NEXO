...
//!
//! Architecturally similar to Z-Image's transformer but with key differences:
...
impl JointAttention {
    ...
    fn forward(
        ...
        // Concatenate in [text, image] order (matches diffusers)
...
/// - AdaLN modulation from timestep embedding: 6 params per stream
#[derive(Debug, Clone)]
...
/// Qwen-Image Transformer 2D Model (BF16 safetensors variant).
///
...
impl QwenImageTransformer2DModel {
    pub fn new(cfg: &QwenImageConfig, vb: VarBuilder) -> candle_core::Result<Self> {
        ...
        // 3D RoPE embedder with Qwen centered positions
        let rope_embedder =
    ...
    /// * `t` - Timestep tensor (B,) -- sigma * 1000
    /// * `encoder_hidden_states` - Text encoder output (B, text_len, 3584)
    /// * `encoder_attention_mask` - Mask (B, text_len)
    ///
    /// # Returns
    ...
    pub fn forward(
        ...
        // 7. Unpack latents: (B, (H/p)*(W/p), out_channels*p*p) -> (B, out_channels, H, W)
        let x_out = img_out
