//! Flux.2 Klein Transformer (diffusers weight format).
//!
...
pub(crate) fn scaled_dot_product_attention(q: &Tensor, k: &Tensor, v: &Tensor) -> Result<Tensor> {
    ...
    if q.device().is_metal() {
        ...
        let attn_weights = (q.matmul(&k.transpose(2, 3)?)? * scale_factor)?;
...
pub enum Flux2TransformerWrapper {
...
impl Flux2TransformerWrapper {
    ...
    pub fn denoise(
        ...
        for (step, window) in timesteps.windows(2).enumerate() {
            ...
            tracing::info!(step = step + 1, total = total_steps, "denoise step");
        }
        Ok(img)
    }
}
...
mod tests {
    ...
    #[test]
    fn timestep_embedding_dtype_preserved() {
        let dev = candle_core::Device::Cpu;
        let t = Tensor::full(0.5f32, 2, &dev).unwrap();
    ...
    #[test]
    fn timestep_embedding_values_bounded() {
        let dev = candle_core::Device::Cpu;
        ...
        for v in &vals {
            assert!(
                *v >= -1.0 && *v <= 1.0,
                "embedding value {v} outside [-1, 1]"
            );
        }
    }

    #[test]
    fn rope_odd_dim_fails() {
        let dev = candle_core::Device::Cpu;
        let pos = Tensor::zeros((1, 4), DType::F32, &dev).unwrap();
        let result = rope(&pos, 33, 2000);
        assert!(result.is_err());
        let err_msg = result.unwrap_err().to_string();
        assert!(err_msg.contains("odd"), "got: {err_msg}");
    }
}
