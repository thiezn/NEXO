...
use crate::inference::models::gemma4::config::{Gemma4Config, Gemma4VisionConfig};
use crate::inference::models::gemma4::generation;
use crate::inference::models::gemma4::safetensors::audio::AudioModel;
use crate::inference::models::gemma4::safetensors::multimodal::broadcast_embed_to_mask;
use crate::inference::models::gemma4::safetensors::multimodal_embedding::MultimodalEmbedder;
use crate::inference::models::gemma4::safetensors::vision::VisionTower;
use crate::inference::models::gemma4::template::Gemma4Template;
use crate::inference::models::support::prompting::{ChatTemplate, ReasoningMode};
use crate::inference::candle::weights::{find_gguf_file, load_gguf};
...
pub fn chat(state: &mut LoadedState, request: &ChatRequest) -> Result<ChatResponse> {
    ...
    let (raw, prompt_cache_tokens, tokens_generated, inference_time_ms) = generation::generate(
        &mut state.model,
        &state.tokenizer,
        &state.device,
    ...
    state.processed_tokens = prompt_cache_tokens;
...
fn ensure_multimodal(state: &mut LoadedState, model_dir: &Path) -> Result<()> {
    ...
    let vis_dtype = crate::inference::candle::device::gpu_compute_dtype(&state.device);
...
pub fn analyze_image(
    ...
    let dtype = crate::inference::candle::device::gpu_compute_dtype(&state.device);
    let (pixel_values, img_h, img_w) =
        crate::inference::models::gemma4::safetensors::pipeline::preprocess_image(
            &image,
            &vis_cfg,
            &state.device,
            dtype,
    ...
    let num_image_tokens = crate::inference::models::gemma4::safetensors::pipeline::compute_num_image_tokens(
        img_h,
        img_w,
...
pub fn analyze_audio(
    ...
    let (mel_tensor, mask_tensor, num_mel_frames) =
        crate::inference::models::gemma4::safetensors::pipeline::prepare_audio(
            &request.pcm_samples,
            request.sample_rate,
            &state.device,
            DType::F32,
    ...
    let num_audio_tokens =
        crate::inference::models::gemma4::safetensors::pipeline::compute_num_audio_tokens(num_mel_frames, 1);
    tracing::debug!(
