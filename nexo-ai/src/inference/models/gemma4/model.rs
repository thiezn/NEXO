...
macro_rules! dispatch {
    ($self:expr, $method:ident $(, $arg:expr)*) => {
        match $self {
            LoadedVariant::Safetensors(state) => state.$method($($arg),*),
            LoadedVariant::Gguf(state) => state.$method($($arg),*),
        }
    };
}
...
impl KvCacheable for Gemma4Model {
    fn kv_cache_seq_len(&self) -> usize {
        self.loaded
            .as_ref()
            .map(|variant| dispatch!(variant, kv_cache_seq_len))
            .unwrap_or(0)
    }
    ...
    fn save_kv_cache(&self) -> Result<Vec<LayerKvSnapshot>> {
        ...
        dispatch!(variant, save_kv_cache).map_err(|error| anyhow::anyhow!("{error}"))
    }
    ...
    fn restore_kv_cache(&mut self, snapshots: &[LayerKvSnapshot]) -> Result<()> {
        let variant = self
            ...
            .as_mut()
            .ok_or_else(|| anyhow::anyhow!("model not loaded"))?;
        dispatch!(variant, restore_kv_cache, snapshots)
            .map_err(|error| anyhow::anyhow!("{error}"))
    }

    fn clear_kv_cache(&mut self) {
        if let Some(variant) = &mut self.loaded {
            dispatch!(variant, clear_kv_cache);
        }
    }
    ...
    fn processed_tokens(&self) -> &[u32] {
        self.loaded
            .as_ref()
            .map(|variant| dispatch!(variant, processed_tokens))
            .unwrap_or(&[])
    }
    ...
    fn current_session_id(&self) -> Option<&str> {
        self.loaded
            .as_ref()
            .and_then(|variant| dispatch!(variant, current_session_id))
    }

    fn set_session_state(&mut self, session_id: Option<String>, tokens: Vec<u32>) {
        if let Some(variant) = &mut self.loaded {
            dispatch!(variant, set_session_state, session_id, tokens);
        }
    }
