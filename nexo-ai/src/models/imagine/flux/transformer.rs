//! Flux.2 Transformer (diffusers weight format)
//!
...
impl SingleStreamBlock {
    fn new(cfg: &Flux2Config, vb: VarBuilder) -> Result<Self> {
        ...
        let linear2 =
            candle_nn::linear_no_bias(h_sz + mlp_sz, h_sz, attn_vb.pp("to_out"))?;
        Ok(Self {
            linear1,
            linear2,
...
impl Flux2TransformerWrapper {
    ...
    pub fn denoise(
        ...
        for (step, window) in timesteps.windows(2).enumerate() {
            ...
            tracing::debug!("denoise step {}/{}", step + 1, total_steps);
        }
        Ok(img)
    }
}
