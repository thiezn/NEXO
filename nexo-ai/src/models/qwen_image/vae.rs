...
impl QwenImageVae {
    pub fn load(vae_path: &std::path::Path, device: &candle_core::Device, dtype: DType) -> Result<Self> {
        let path_str = vae_path
            .to_str()
            .ok_or_else(|| candle_core::Error::Msg("invalid VAE path".to_string()))?;
        let vb =
            unsafe { candle_nn::VarBuilder::from_mmaped_safetensors(&[path_str], dtype, device)? };
...
mod tests {
    ...
    fn test_latent_std_all_positive() {
        for (i, &val) in LATENTS_STD.iter().enumerate() {
            assert!(val > 0.0, "LATENTS_STD[{i}] = {val} is not positive");
        }
    }

    #[test]
    fn test_block_out_channels_architecture() {
        assert_eq!(BLOCK_OUT_CHANNELS.len(), 4);
        assert_eq!(BLOCK_OUT_CHANNELS, [96, 192, 384, 384]);
        for i in 1..BLOCK_OUT_CHANNELS.len() {
            assert!(BLOCK_OUT_CHANNELS[i] >= BLOCK_OUT_CHANNELS[i - 1]);
        }
    }
}
