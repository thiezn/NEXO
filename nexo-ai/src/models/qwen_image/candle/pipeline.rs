...
use crate::models::support::encoders::qwen2_text::Qwen2TextEncoder;
...
impl ModelFiles {
    fn discover(model_dir: &Path) -> Result<Self> {
        ...
        let (transformer_paths, is_quantized) = if transformer_dir.exists() {
            ...
            let paths = crate::models::support::weights::find_safetensor_files(&transformer_dir)?;
            (paths, false)
        } else {
            // Look for GGUF file in model root
            let gguf = crate::models::support::weights::find_gguf_file(
                model_dir,
                "",
                &["vae", "text_encoder"],
            );
            match gguf {
                ...
                Err(_) => {
                    ...
                    let paths = crate::models::support::weights::find_safetensor_files(model_dir)?;
                    (paths, false)
                }
            }
        };
        ...
        let vae_path = if vae_dir.exists() {
            let files = crate::models::support::weights::find_safetensor_files(&vae_dir)?;
            files
                .into_iter()
                .next()
        ...
        let text_encoder_paths = if te_dir.exists() {
            crate::models::support::weights::find_safetensor_files(&te_dir)?
        } else {
            bail!(
