...
fn find_transformer(model_dir: &Path) -> Result<(Vec<PathBuf>, bool)> {
    ...
    if xformer_dir.is_dir() {
        let files = crate::models::support::weights::find_safetensor_files(&xformer_dir)?;
...
fn find_text_encoder(model_dir: &Path) -> Result<Vec<PathBuf>> {
    ...
    if enc_dir.is_dir() {
        return crate::models::support::weights::find_safetensor_files(&enc_dir);
    }
...
pub fn imagine(state: &LoadedState, request: &ImagineRequest) -> Result<ImagineResponse> {
    ...
    let encoder = crate::models::support::encoders::qwen3::Qwen3Encoder::load(
        &encoder_paths,
        &tokenizer_path,
        enc_device,
        enc_dtype,
