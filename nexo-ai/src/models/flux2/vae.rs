...
mod tests {
    ...
    #[test]
    fn decoder_channel_progression() {
        let cfg = Flux2VaeConfig::klein();
        assert_eq!(cfg.block_out_channels.len(), 4);
        assert_eq!(cfg.block_out_channels[2], cfg.block_out_channels[3]);
    }

    #[test]
    fn patchify_non_square() {
        let dev = Device::Cpu;
        let b = 1usize;
        let c = 32usize;
        let h = 8usize;
        ...
        assert_eq!(patched.dims(), &[1, 128, 4, 8]);
    }

    #[test]
    fn bn_denormalization_zero_variance() {
        let dev = Device::Cpu;
        let eps = 0.0001f64;
        ...
        let vals: Vec<f32> = result.flatten_all().unwrap().to_vec1().unwrap();
        let expected_std = (eps as f32).sqrt();
        for (i, &v) in vals.iter().enumerate() {
            assert!(v.is_finite(), "channel {i} produced non-finite value");
            assert!(
