...
/// | Token | ID | Purpose |
/// |---|---|---|
/// | `<\|turn>` | 105 | Beginning of a dialogue turn |
/// | `<turn\|>` | 106 | End of a dialogue turn |
/// | `system` | — | System instruction role |
/// | `user` | — | User turn role |
/// | `model` | — | Model turn role |
///
/// # Multimodal tokens
///
/// | Token | Purpose |
/// |---|---|
/// | `<\|image>` / `<image\|>` | Image embedding wrapper |
/// | `<\|audio>` / `<audio\|>` | Audio embedding wrapper |
/// | `<\|image\|>` / `<\|audio\|>` | Placeholder tokens (replaced by embeddings) |
///
/// # Tool tokens
///
/// | Token | Purpose |
...
/// | `<\|"\|>` | String delimiter in structured blocks |
///
/// # Thinking tokens
///
/// | Token | Purpose |
/// |---|---|
/// | `<\|think\|>` | Activates thinking mode (in system instruction) |
/// | `<\|channel>` / `<channel\|>` | Model's internal reasoning channel |
///
/// # Prompt format
///
/// ```text
/// <|turn>system
/// {system_instruction}<|tool>declaration:name{...}<tool|><turn|>
/// <|turn>user
/// {content}<turn|>
/// <|turn>model
/// {content}<turn|>
...
impl ChatTemplate for Gemma4Template {
    fn format_prompt(&self, messages: &[ChatMessage], _reasoning: &ReasoningMode) -> String {
        ...
        for (i, msg) in messages.iter().enumerate() {
            let role = match msg.role {
                ...
                ChatRole::Tool => {
                    // Tool responses are injected inside the preceding model turn.
                    // If the previous message was an Assistant turn we merge; otherwise
                    // we wrap in a model turn ourselves.
                    let needs_turn = i == 0
                    ...
                    prompt.push_str("<tool_response|>");
                    // Don't close the model turn — the next model message will continue it,
                    // or we'll close it below if we opened it ourselves and no model follows.
                    if needs_turn {
            ...
            // For assistant messages that are followed by a tool response,
            // don't close the turn — the tool response merges into it.
    ...
    fn format_with_tools(
        ...
        // Build tool declarations in Gemma 4 native format
        let mut tool_decls = String::new();
        for tool in tools {
            tool_decls.push_str(&format_tool_declaration(tool));
        }

        // Build system instruction with tool declarations
    ...
    fn parse_response(&self, raw: &str) -> String {
        ...
        // Strip thinking channel blocks: <|channel>thought\n...\n<channel|>
    ...
    fn parse_tool_calls(&self, raw: &str) -> (Vec<ToolCall>, Option<String>) {
        ...
        // Strip thinking channel and extract reasoning
        ...
        // Try native Gemma 4 format: <|tool_call>call:name{...}<tool_call|>
...
/// Format a `ToolSpec` as a Gemma 4 native tool declaration.
///
/// Output: `<|tool>declaration:name{description:<|"|>...<|"|>,parameters:{...}}<tool|>`
fn format_tool_declaration(tool: &nexo_spec::tool::ToolSpec) -> String {
    ...
    // Parameters — extract "properties" from JSON schema
...
/// Wrap a string value in Gemma 4 string delimiters: `<|"|>value<|"|>`
...
/// Parse `<|tool_call>call:name{...}<tool_call|>` blocks from model output.
...
/// The `<|"|>` delimiter as bytes — all ASCII, safe for byte-level scanning.
const STR_DELIM: &[u8] = b"<|\"|>";

/// Check if `bytes[pos..]` starts with `needle`.
...
/// Parse Gemma 4 key-value pairs: `key:<|"|>value<|"|>,key2:123`
///
/// All delimiter scanning is byte-level so multi-byte UTF-8 in values is safe.
fn parse_gemma4_kv_pairs(input: &str) -> serde_json::Map<String, serde_json::Value> {
    ...
    while pos < len {
        ...
        // Read key (until ':') — keys are ASCII identifiers
        let key_start = pos;
        ...
        if bytes_match_at(bytes, pos, STR_DELIM) {
            // String value delimited by <|"|> — scan bytes to handle multi-byte UTF-8
            pos += STR_DELIM.len();
            let str_start = pos;
            ...
            // Nested object — brace-match at byte level
            let mut depth = 0;
            let obj_start = pos;
            while pos < len {
                match bytes[pos] {
            ...
            // Bare value: number, bool, or null — read until ',' or '}'
            let val_start = pos;
...
mod tests {
    ...
    fn parse_response_strips_turn_marker() {
        let raw = "Hello there!<turn|>";
        assert_eq!(template().parse_response(raw), "Hello there!");
    }

    #[test]
    fn parse_response_strips_tool_response_marker() {
        let raw = "I'll check that.<|tool_response>";
        assert_eq!(template().parse_response(raw), "I'll check that.");
    }
    ...
    fn parse_native_tool_call_utf8_value() {
        // Multi-byte UTF-8 in tool call values must not panic
        let raw =
    ...
    #[test]
    fn format_tool_declaration_basic() {
        let tool = nexo_spec::tool::ToolSpec {
        ...
        let decl = format_tool_declaration(&tool);
        assert!(decl.starts_with("<|tool>declaration:get_weather{"));
        assert!(decl.contains("description:<|\"|>Get weather for a location<|\"|>"));
        assert!(decl.contains("location:{type:<|\"|>string<|\"|>"));
        assert!(decl.ends_with("<tool|>"));
    }
    ...
    fn end_of_turn_markers() {
        ...
        assert!(markers.contains(&"<turn|>"));
        assert!(markers.contains(&"<|tool_response>"));
    }
    ...
    fn tool_response_merges_into_model_turn() {
        ...
        let result = template().format_prompt(&msgs, &ReasoningMode::Disabled);
        // Assistant turn should NOT be closed before tool response
