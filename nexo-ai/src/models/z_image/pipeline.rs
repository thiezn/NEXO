...
fn discover_files(model_dir: &Path) -> Result<ModelFiles> {
    ...
    if !xformer_dir.exists() {
        bail!(
            "transformer directory not found: {}",
            xformer_dir.display()
        );
    }

    let (transformer_paths, is_gguf) =
        if let Ok(gguf) = find_gguf_file(&xformer_dir, "", &[]) {
            (vec![gguf], true)
        } else {
...
pub fn load(model_dir: &Path) -> Result<LoadedState> {
    ...
    tracing::info!("Z-Image model files validated in {:.1}s", start.elapsed().as_secs_f64());

    let device = device::create_device(|msg| tracing::info!("{msg}"))?;
...
pub fn imagine(state: &mut LoadedState, request: &ImagineRequest) -> Result<ImagineResponse> {
    ...
    tracing::info!("loading Qwen3 text encoder ({} shards)", files.encoder_paths.len());
    let encode_start = Instant::now();

    let encoder = Qwen3Encoder::load(&files.encoder_paths, &files.tokenizer_path, device, dtype)?;
    tracing::info!("Qwen3 encoder loaded in {:.1}s", encode_start.elapsed().as_secs_f64());
    ...
    let transformer = if files.is_gguf {
        ...
        ZImageTransformer::Quantized(
            QuantizedZImageTransformer2DModel::new(&transformer_cfg, dtype, vb)?,
        )
    } else {
    ...
    let vae_dtype = if vae_device.is_cpu() { DType::F32 } else { dtype };
    ...
    let vae_vb = unsafe {
        VarBuilder::from_mmaped_safetensors(&[vae_path_str], vae_dtype, &vae_device)?
    };
