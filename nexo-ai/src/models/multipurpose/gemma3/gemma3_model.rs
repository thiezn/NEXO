//! Vendored from candle-transformers 0.9.2 gemma3.rs with fix for non-contiguous
//! value_states causing "slice-set only supports contiguous tensors" in KV cache.
...
impl Attention {
    ...
    fn forward(
        ...
        let key_states = key_states
            ...
            .transpose(1, 2)?;
        // FIX: make value_states contiguous after transpose so KV cache slice_set works.
...
impl Model {
    ...
    pub fn forward(&mut self, input_ids: &Tensor, seqlen_offset: usize) -> Result<Tensor> {
        ...
        self.forward_inner(xs, b_size, seqlen_offset)
    }

    /// Forward pass starting from pre-computed embeddings (for vision+text merge).
    pub fn forward_embeds(&mut self, xs: &Tensor, seqlen_offset: usize) -> Result<Tensor> {
        let (b_size, _seq_len, _) = xs.dims3()?;
        self.forward_inner(xs.clone(), b_size, seqlen_offset)
    }
    ...
    /// Get the embedding for a set of token IDs (for vision pipeline token merge).
