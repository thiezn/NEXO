...
pub fn detect_available_models() -> Vec<String> {
    ...
    known_manifests()
        .par_iter()
        .filter_map(|manifest| match manifest_rejection(manifest, &mdir) {
            None => Some(manifest.manifest.name.clone()),
            Some(rejection) => {
                log_manifest_rejection(manifest, &mdir, &rejection);
                None
            }
        })
        .collect()
...
#[cfg(feature = "download")]
fn manifest_rejection(
    manifest: &AiModelManifest,
    models_dir: &std::path::Path,
) -> Option<ManifestRejection> {
...
#[cfg(feature = "download")]
fn manifest_has_local_artifacts(manifest: &AiModelManifest, models_dir: &std::path::Path) -> bool {
    use crate::download::manifest::storage_path;
