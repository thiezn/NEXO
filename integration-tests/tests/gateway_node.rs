...
use nexo_ws_schema::{
    Frame, Method, ModelStatusParams, Platform, StatusResponse, ToolSpecEntry,
    ToolsRegisterParams,
};
...
async fn request_status(conn: &mut NexoConnection) -> StatusResponse {
    let frame =
        Frame::request(Method::Status, &serde_json::json!({})).expect("build status frame");
