...
- Update the events/status messages between nexo-node and gateway. The nexo-node should periodically, and on each change, update it's current state, that includes tools, model loaded, active session_id in kv cache, etc.
