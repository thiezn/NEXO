...
use crate::inference::models::gemma4::generation::TextForward;
...
impl TextForward for QuantizedTextModel {
    ...
    fn clear_kv_cache(&mut self) {
        QuantizedTextModel::clear_kv_cache(self);
    }

    fn save_kv_cache(&mut self) -> Result<Vec<LayerKvSnapshot>> {
        QuantizedTextModel::save_kv_cache(self)
    }

    fn restore_kv_cache(&mut self, snapshots: &[LayerKvSnapshot]) -> Result<()> {
        QuantizedTextModel::restore_kv_cache(self, snapshots)
    }
}
