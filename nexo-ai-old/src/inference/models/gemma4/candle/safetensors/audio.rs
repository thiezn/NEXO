...
use crate::inference::models::gemma4::config::Gemma4AudioConfig;
...
impl ConformerLightConv1d {
    ...
    fn forward(&self, audio_encodings: &Tensor) -> Result<Tensor> {
        ...
        let x = x
            .to_dtype(DType::F32)?
            .conv1d(
                &self.depthwise_conv1d.weight().to_dtype(DType::F32)?,
                self.depthwise_conv1d.config().padding,
                self.depthwise_conv1d.config().stride,
                self.depthwise_conv1d.config().dilation,
                self.depthwise_conv1d.config().groups,
            )?
            .to_dtype(audio_encodings.dtype())?
