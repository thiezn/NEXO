...
use crate::inference::models::gemma4::config::Gemma4TextConfig;
...
fn reorder_sliding_snapshot_for_restore(
    data: &Tensor,
    snapshot: &LayerKvSnapshot,
) -> Result<Tensor> {
    let seq_len = data.dim(2)?;
    if !snapshot.is_sliding || seq_len == 0 || snapshot.current_seq_len <= seq_len {
        return Ok(data.clone());
    }

    let rotate_at = snapshot.offset % seq_len;
    if rotate_at == 0 {
        return Ok(data.clone());
    }

    let tail = data.narrow(2, rotate_at, seq_len - rotate_at)?;
    let head = data.narrow(2, 0, rotate_at)?;
    ...
    pub fn forward_embeds(
        ...
        for (i, layer) in layers.iter().enumerate() {
            ...
            let ple_slice = per_layer_inputs
                .map(|pli| {
                    pli.narrow(pli.dims().len() - 2, i, 1)?
                        .squeeze(pli.dims().len() - 2)
                })
                .transpose()?;
            xs = layer.forward(
                &xs,
        ...
        for snap in snapshots {
            if snap.layer_idx < self.kv_caches.len() {
                ...
                if let (Some(k), Some(v)) = (&snap.k_data, &snap.v_data) {
                    let k = reorder_sliding_snapshot_for_restore(k, snap)?;
                    let v = reorder_sliding_snapshot_for_restore(v, snap)?;
                    match cache {
                        KvCache::Normal(c) => {
...
mod tests {
    ...
    fn token_tensor(values: &[f32]) -> Tensor {
    ...
    #[test]
    fn wrapped_sliding_snapshot_is_reordered_before_restore() {
        let mut cache = candle_nn::kv_cache::RotatingKvCache::new(2, 4);
        let first = token_tensor(&[0.0, 1.0, 2.0, 3.0]);
        let second = token_tensor(&[4.0, 5.0]);

        cache.k_cache_mut().append(&first).unwrap();
        cache.v_cache_mut().append(&first).unwrap();
        cache.k_cache_mut().append(&second).unwrap();
        cache.v_cache_mut().append(&second).unwrap();

        let snapshot = LayerKvSnapshot {
            layer_idx: 0,
            is_sliding: true,
            k_data: cache.k().unwrap(),
            v_data: cache.v().unwrap(),
            offset: cache.offset(),
            current_seq_len: cache.current_seq_len(),
        };

        assert_eq!(snapshot.offset, 2);
        assert_eq!(snapshot.current_seq_len, 6);
        assert_eq!(
            tensor_values(snapshot.k_data.as_ref().unwrap()),
            vec![4.0, 5.0, 2.0, 3.0]
        );

        let reordered =
            reorder_sliding_snapshot_for_restore(snapshot.k_data.as_ref().unwrap(), &snapshot)
                .unwrap();

        assert_eq!(tensor_values(&reordered), vec![2.0, 3.0, 4.0, 5.0]);

        let mut restored = candle_nn::kv_cache::RotatingKvCache::new(2, 4);
        restored.k_cache_mut().append(&reordered).unwrap();

        assert_eq!(restored.offset(), 0);
        assert_eq!(
            tensor_values(&restored.k().unwrap().unwrap()),
            vec![2.0, 3.0, 4.0, 5.0]
        );
    }
}
