...
use crate::inference::models::support::encoders::qwen2_text::Qwen2TextEncoder;
...
impl ModelFiles {
    fn discover(model_dir: &Path) -> Result<Self> {
        ...
        let (transformer_paths, is_quantized) = if transformer_dir.exists() {
            ...
            let paths = crate::inference::candle::weights::find_safetensor_files(&transformer_dir)?;
            (paths, false)
        } else {
            // Look for GGUF file in model root
            let gguf = crate::inference::candle::weights::find_gguf_file(
                model_dir,
                "",
                &["vae", "text_encoder"],
            );
            match gguf {
                ...
                Err(_) => {
                    ...
                    let paths = crate::inference::candle::weights::find_safetensor_files(model_dir)?;
                    (paths, false)
                }
            }
        };
        ...
        let text_encoder_paths = if te_dir.exists() {
            crate::inference::candle::weights::find_safetensor_files(&te_dir)?
        } else {
            bail!(
...
fn resolve_vae_device(gpu_device: &Device) -> Device {
    ...
    let available = crate::inference::candle::device::available_system_memory_bytes().unwrap_or(0);
    if available > VAE_DECODE_MEMORY_THRESHOLD {
        gpu_device.clone()
    } else {
        tracing::info!(
            "insufficient memory for VAE on GPU ({} available), using CPU",
            crate::inference::candle::device::fmt_gb(available)
        );
        Device::Cpu
    }
}
