...
//!
//! Loads GGUF weights, dequantizes on CPU to target dtype, moves to GPU, then
//! uses standard `candle_nn::Linear` for inference. This produces identical
//! precision to the BF16 safetensors path since all inference runs through
//! BF16/F16 matmul.
//!
...
// ---------------------------------------------------------------------------
// Utility: dequantize GGUF -> target dtype on CPU -> move to GPU
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Building blocks (all use candle_nn::Linear, target dtype on GPU)
// ---------------------------------------------------------------------------
...
impl QLastLayer {
    ...
    fn forward(&self, xs: &Tensor, vec: &Tensor) -> Result<Tensor> {
        ...
        // CRITICAL: BFL format — shift first, scale second (opposite of diffusers format)
...
///
/// Weights are dequantized on CPU to target dtype, then moved to GPU. Inference
/// uses standard `candle_nn::Linear` matmul, giving identical precision to the
/// BF16 safetensors path. GGUF advantage: smaller download (Q8: ~4.3GB vs ~7.7GB).
pub struct QuantizedFlux2Transformer {
    img_in: Linear,
    txt_in: Linear,
    time_in: MlpEmbedder,
    pe_embedder: EmbedNd,
...
mod tests {
    ...
    fn qlast_layer_uses_bfl_shift_scale_ordering() {
        ...
        // ada_ln weight: first h_sz rows = identity, second h_sz rows = zeros.
        // So chunks[0] approx silu(1)*1 = 0.73, chunks[1] = 0.
        ...
        // Non-uniform input so norm(xs) != 1
        ...
        // Verify BFL ordering: output consistent with additive shift
