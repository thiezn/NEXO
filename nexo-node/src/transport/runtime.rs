use super::inference::{
    ...
    load_startup_models, push_model_status, queue_audio_analyze, queue_audio_generate,
    queue_image_analyze, queue_image_generate, queue_run_round, shared_models,
};
    ...
    match frame {
        ...
        Some(Frame::Request {
            ...
            if *context.inference_busy {
                ...
                *context.inference_busy = true;
                queue_image_generate(&id, params, context.models, context.inference_tx).await?;
            }
            Ok(MessageLoopAction::Continue)
        }
        Some(Frame::Request {
            id,
            method: Method::AudioGenerate,
            params,
        }) => {
            if *context.inference_busy {
                send_busy_error(writer, &id).await?;
                ...
                *context.inference_busy = true;
                queue_audio_generate(&id, params, context.models, context.inference_tx).await?;
            }
            Ok(MessageLoopAction::Continue)
        }
        Some(Frame::Request {
