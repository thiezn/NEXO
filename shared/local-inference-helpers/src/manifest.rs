...
/// Trait for model component identifiers. Each consumer crate provides its own enum.
///
/// ```ignore
/// #[derive(Clone, Debug)]
/// enum ImageComponent { Transformer, Vae, T5Encoder, ... }
///
/// impl Component for ImageComponent {
///     fn name(&self) -> &str { match self { Self::Transformer => "transformer", ... } }
///     fn is_model_specific(&self) -> bool { matches!(self, Self::Transformer) }
/// }
/// ```
pub trait Component: Clone + std::fmt::Debug + Send + Sync + 'static {
    /// Short identifier used as storage key (e.g. "transformer", "vae").
    fn name(&self) -> &str;

    /// Whether this component is model-specific (stored per-model) or shared across models
    /// of the same family (stored under `shared/<family>/`).
...
/// Default inference settings for a model.
#[derive(Debug, Clone)]
...
/// A complete model definition: identity, files to download, and defaults.
#[derive(Debug, Clone)]
