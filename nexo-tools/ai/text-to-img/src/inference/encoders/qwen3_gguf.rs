...
const KV_REPEAT: usize = N_HEADS / N_KV_HEADS;
...
impl SwiGluFFN {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        let gate_out =
            local_inference_helpers::candle_nn::Activation::Silu.forward(&self.gate.forward(xs)?)?;
...
impl Qwen3Attention {
    fn forward(
        ...
        let attn_weights =
            local_inference_helpers::candle_nn::ops::softmax_last_dim(&scores)?;
