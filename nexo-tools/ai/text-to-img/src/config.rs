...
/// Application configuration, stored at `~/.nexo/text_to_img.toml`.
...
impl AppConfig {
    /// Config file path: `~/.nexo/text_to_img.toml`.
...
impl ImageModelPaths {
    ...
    pub fn from_downloads(
        downloads: &[(crate::manifest::ImageComponent, PathBuf)],
    ) -> Option<Self> {
    ...
    pub fn to_model_config(
        ...
        family: &str,
        description: &str,
        defaults: &local_inference_helpers::manifest::ManifestDefaults,
        is_schnell: bool,
    ) -> ModelConfig {
        ModelConfig {
