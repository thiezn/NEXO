...
pub fn read_resource_header(
    data: &[u8],
    offset: usize,
    vol_version: ResVolVersion,
    ...
    match vol_version {
        ResVolVersion::Sci0 => {
            ...
            if remaining < 8 {
                anyhow::bail!(
                    "Not enough data for SCI0 resource header at offset {}",
                    offset
                );
            }
            let d = &data[offset..];
        ...
        ResVolVersion::Sci1Early => {
            ...
            if remaining < 9 {
                anyhow::bail!(
                    "Not enough data for SCI1 resource header at offset {}",
                    offset
                );
            }
            let d = &data[offset..];
        ...
        ResVolVersion::Sci1Late => {
            ...
            if remaining < 9 {
                anyhow::bail!(
                    "Not enough data for SCI1Late resource header at offset {}",
                    offset
                );
            }
            let d = &data[offset..];
        ...
        ResVolVersion::Sci11 => {
            ...
            if remaining < 9 {
                anyhow::bail!(
                    "Not enough data for SCI1.1 resource header at offset {}",
                    offset
                );
            }
            let d = &data[offset..];
        ...
        ResVolVersion::Sci2 => {
            ...
            if remaining < 13 {
                anyhow::bail!(
                    "Not enough data for SCI2 resource header at offset {}",
                    offset
                );
            }
            let d = &data[offset..];
...
pub fn read_resource_data<'a>(
    ...
    if data_end > data.len() {
        anyhow::bail!(
            "Resource data extends beyond volume file (offset {}, packed_size {})",
            offset,
            header.packed_size
        );
    }
