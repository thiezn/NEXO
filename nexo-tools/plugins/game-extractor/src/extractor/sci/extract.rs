...
use crate::extractor::common::output::{
    OutputManager, PaletteImage, sanitize_name, save_image, write_jsonl,
};
...
pub fn extract_game(
    ...
    let pic_results: Vec<_> = pic_numbers
        .iter()
        .filter_map(|&num| {
    ...
    let global_non_black = global_palette
        .iter()
        .filter(|c| c[0] != 0 || c[1] != 0 || c[2] != 0)
        .count();
    if global_non_black < MIN_PALETTE_COLORS {
        if let Some(pic_pal) = best_pic_palette {
            log.push(format!(
                "{}: using pic palette for views ({} -> {} non-black colors)",
                prefix,
                global_non_black,
                pic_pal
                    .iter()
    ...
    let view_results: Vec<_> = view_numbers
        .iter()
        .filter_map(|&num| {
    ...
    let sound_results: Vec<_> = sound_numbers
        .iter()
        .filter_map(|&num| {
    ...
    log.push(format!(
        "{}: extracted {} backgrounds, {} view sprites, {} sounds",
...
fn extract_view(
    ...
    for (loop_idx, vloop) in view_res.loops.iter().enumerate() {
        ...
        let cel_refs: Vec<&view::ViewCel> = vloop
            .cels
            .iter()
        ...
        if !cel_refs.is_empty() {
            ...
            if sw > 0 && sh > 0 {
                ...
                lora_entries.push(LoraEntry {
                    image: format!("images/{}", lora_filename),
                    text: format!(
                        "a pixel art sprite sheet, view {} loop {} animation",
                        number, loop_idx
                    ),
                });
            }
