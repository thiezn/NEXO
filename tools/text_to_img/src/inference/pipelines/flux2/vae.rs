...
//! - BatchNorm2d latent denormalization

use local_inference_helpers::candle_core::{Result, Tensor, D};
use local_inference_helpers::candle_nn::{
    conv2d, group_norm, Conv2d, Conv2dConfig, GroupNorm, Linear, Module, VarBuilder,
};
...
fn scaled_dot_product_attention(q: &Tensor, k: &Tensor, v: &Tensor) -> Result<Tensor> {
    ...
    local_inference_helpers::candle_nn::ops::softmax_last_dim(&attn_weights)?.matmul(v)
}
...
impl AttnBlock {
    fn new(in_c: usize, num_groups: usize, vb: VarBuilder) -> Result<Self> {
        Ok(Self {
            to_q: local_inference_helpers::candle_nn::linear(in_c, in_c, vb.pp("to_q"))?,
            to_k: local_inference_helpers::candle_nn::linear(in_c, in_c, vb.pp("to_k"))?,
            to_v: local_inference_helpers::candle_nn::linear(in_c, in_c, vb.pp("to_v"))?,
            to_out: local_inference_helpers::candle_nn::linear(in_c, in_c, vb.pp("to_out").pp("0"))?,
...
impl Module for AttnBlock {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let xs = xs.squeeze(1)?.apply(&self.to_out)?;
...
impl Module for ResnetBlock {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        let h = xs
            .apply(&self.norm1)?
            .apply(&local_inference_helpers::candle_nn::Activation::Swish)?
            .apply(&self.conv1)?
            .apply(&self.norm2)?
            .apply(&local_inference_helpers::candle_nn::Activation::Swish)?
            .apply(&self.conv2)?;
...
impl Flux2AutoEncoder {
    ...
    /// Input: (B, 32, H, W) -> Output: (B, 3, H*8, W*8)
