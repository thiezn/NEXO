...
//!
//! Architecture: DoubleStreamBlock + SingleStreamBlock with:
...
//! - Shared modulation across all blocks
//! - 5 double + 20 single blocks for Klein-4B

use local_inference_helpers::candle_core::{DType, IndexOp, Result, Tensor, D};
use local_inference_helpers::candle_nn::{LayerNorm, Linear, RmsNorm, VarBuilder};
...
fn scaled_dot_product_attention(q: &Tensor, k: &Tensor, v: &Tensor) -> Result<Tensor> {
    ...
    let attn_scores = local_inference_helpers::candle_nn::ops::softmax_last_dim(&attn_weights)?
        .matmul(&v)?;
...
fn rope(pos: &Tensor, dim: usize, theta: usize) -> Result<Tensor> {
    if dim % 2 == 1 {
        local_inference_helpers::candle_core::bail!("dim {dim} is odd")
    }
    let dev = pos.device();
...
fn timestep_embedding(t: &Tensor, dim: usize, dtype: DType) -> Result<Tensor> {
    ...
    if dim % 2 == 1 {
        local_inference_helpers::candle_core::bail!("{dim} is odd")
    }
    let dev = t.device();
    let half = dim / 2;
    let t = (t * TIME_FACTOR)?;
...
impl local_inference_helpers::candle_core::Module for EmbedNd {
...
impl MlpEmbedder {
    fn new(in_sz: usize, h_sz: usize, vb: VarBuilder) -> Result<Self> {
        let in_layer = local_inference_helpers::candle_nn::linear_no_bias(in_sz, h_sz, vb.pp("linear_1"))?;
        let out_layer = local_inference_helpers::candle_nn::linear_no_bias(h_sz, h_sz, vb.pp("linear_2"))?;
        Ok(Self { in_layer, out_layer })
    }
}

impl local_inference_helpers::candle_core::Module for MlpEmbedder {
...
impl Modulation1 {
    fn new(dim: usize, vb: VarBuilder) -> Result<Self> {
        let lin = local_inference_helpers::candle_nn::linear_no_bias(dim, 3 * dim, vb.pp("linear"))?;
        Ok(Self { lin })
    }
    ...
    fn forward(&self, vec_: &Tensor) -> Result<ModulationOut> {
        let ys = vec_
            ...
            .chunk(3, D::Minus1)?;
        if ys.len() != 3 {
            local_inference_helpers::candle_core::bail!("unexpected len from chunk {ys:?}")
        }
        Ok(ModulationOut {
            shift: ys[0].clone(),
            scale: ys[1].clone(),
...
impl Modulation2 {
    fn new(dim: usize, vb: VarBuilder) -> Result<Self> {
        let lin = local_inference_helpers::candle_nn::linear_no_bias(dim, 6 * dim, vb.pp("linear"))?;
        Ok(Self { lin })
    }
...
impl Mlp {
    fn new(in_sz: usize, mlp_sz: usize, vb: VarBuilder) -> Result<Self> {
        let lin1 = local_inference_helpers::candle_nn::linear_no_bias(in_sz, mlp_sz * 2, vb.pp("linear_in"))?;
        let lin2 = local_inference_helpers::candle_nn::linear_no_bias(mlp_sz, in_sz, vb.pp("linear_out"))?;
        Ok(Self { lin1, lin2, mlp_sz })
    }
}

impl local_inference_helpers::candle_core::Module for Mlp {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
...
impl DoubleAttention {
    fn new_img(dim: usize, num_heads: usize, vb: VarBuilder) -> Result<Self> {
        ...
        Ok(Self {
            to_q: local_inference_helpers::candle_nn::linear_no_bias(dim, dim, vb.pp("to_q"))?,
            to_k: local_inference_helpers::candle_nn::linear_no_bias(dim, dim, vb.pp("to_k"))?,
            to_v: local_inference_helpers::candle_nn::linear_no_bias(dim, dim, vb.pp("to_v"))?,
            to_out: local_inference_helpers::candle_nn::linear_no_bias(dim, dim, vb.pp("to_out").pp("0"))?,
    ...
    fn new_txt(dim: usize, num_heads: usize, vb: VarBuilder) -> Result<Self> {
        let head_dim = dim / num_heads;
        Ok(Self {
            to_q: local_inference_helpers::candle_nn::linear_no_bias(dim, dim, vb.pp("add_q_proj"))?,
            to_k: local_inference_helpers::candle_nn::linear_no_bias(dim, dim, vb.pp("add_k_proj"))?,
            to_v: local_inference_helpers::candle_nn::linear_no_bias(dim, dim, vb.pp("add_v_proj"))?,
            to_out: local_inference_helpers::candle_nn::linear_no_bias(dim, dim, vb.pp("to_add_out"))?,
...
impl SingleStreamBlock {
    fn new(cfg: &Flux2Config, vb: VarBuilder) -> Result<Self> {
        ...
        let attn_vb = vb.pp("attn");
        let linear1 =
            local_inference_helpers::candle_nn::linear_no_bias(h_sz, h_sz * 3 + mlp_sz * 2, attn_vb.pp("to_qkv_mlp_proj"))?;
        let linear2 = local_inference_helpers::candle_nn::linear_no_bias(h_sz + mlp_sz, h_sz, attn_vb.pp("to_out"))?;
        Ok(Self {
            linear1,
            linear2,
...
impl LastLayer {
    fn new(h_sz: usize, out_c: usize, vb: VarBuilder) -> Result<Self> {
        Ok(Self {
            ...
            linear: local_inference_helpers::candle_nn::linear_no_bias(h_sz, out_c, vb.pp("proj_out"))?,
            ada_ln_modulation: local_inference_helpers::candle_nn::linear_no_bias(
                h_sz,
                2 * h_sz,
...
impl Flux2Transformer {
    pub fn new(cfg: &Flux2Config, vb: VarBuilder) -> Result<Self> {
        let img_in =
            local_inference_helpers::candle_nn::linear_no_bias(cfg.in_channels, cfg.hidden_size, vb.pp("x_embedder"))?;
        let txt_in = local_inference_helpers::candle_nn::linear_no_bias(
            cfg.context_in_dim,
            cfg.hidden_size,
    ...
    pub fn forward(
        ...
        if txt.rank() != 3 || img.rank() != 3 {
            local_inference_helpers::candle_core::bail!(
                "expected rank 3, got txt={} img={}",
                txt.rank(),
                img.rank()
            )
        }
...
impl Flux2TransformerWrapper {
    ...
    pub fn denoise(
        ...
        timesteps: &[f64],
        guidance: f64,
        progress: &local_inference_helpers::progress::ProgressReporter,
    ) -> anyhow::Result<Tensor> {
        use local_inference_helpers::progress::ProgressEvent;
...
mod tests {
    ...
    fn timestep_embedding_shape() {
        let dev = local_inference_helpers::candle_core::Device::Cpu;
