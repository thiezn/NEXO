...
use local_inference_helpers::noise::seeded_randn;
...
impl QwenImageEngine {
    fn debug_tensor_stats(name: &str, tensor: &Tensor) {
        if std::env::var_os("MYCLAW_QWEN_DEBUG").is_none() {
            return;
        }
    ...
    fn generate_sequential(&mut self, req: &GenerateRequest) -> Result<GenerateResponse> {
        ...
        let mut latents =
            seeded_randn(seed, &[1, 16, latent_h, latent_w], &device, latent_dtype)?;
...
impl InferenceEngine for QwenImageEngine {
    fn generate(&mut self, req: &GenerateRequest) -> Result<GenerateResponse> {
        ...
        if loaded.text_encoder.needs_reload() {
