...
use local_inference_helpers::candle_core::{DType, Module, Result, Tensor, D};
use local_inference_helpers::candle_nn::RmsNorm as CandleRmsNorm;
use candle_transformers::models::z_image::transformer::{
    apply_rotary_emb, create_coordinate_grid, patchify, unpatchify, LayerNormNoParams,
    RopeEmbedder,
};
...
impl Module for ApproximateGelu {
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        x.broadcast_mul(&local_inference_helpers::candle_nn::ops::sigmoid(&(x.clone() * 1.702)?)?)
    }
}
...
impl JointAttention {
    ...
    fn forward(
        ...
        attn_weights = local_inference_helpers::candle_nn::ops::softmax_last_dim(&attn_weights)?;
