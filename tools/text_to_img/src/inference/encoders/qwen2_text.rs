...
use anyhow::Result;
use local_inference_helpers::candle_core::{DType, Device, IndexOp, Module, Tensor};
...
impl RotaryEmbedding {
    ...
    fn apply_rotary_emb_qkv(
        ...
        let q_embed =
            local_inference_helpers::candle_nn::rotary_emb::rope(&q.contiguous()?, &cos, &sin)?;
        let k_embed =
            local_inference_helpers::candle_nn::rotary_emb::rope(&k.contiguous()?, &cos, &sin)?;
        Ok((q_embed, k_embed))
    }
}
...
struct Mlp {
    ...
    act_fn: local_inference_helpers::candle_nn::Activation,
}
...
impl Mlp {
    fn new(cfg: &Qwen2TextEncoderConfig, vb: VarBuilder) -> Result<Self> {
        ...
        Ok(Self {
            ...
            up_proj,
            down_proj,
            act_fn: local_inference_helpers::candle_nn::Activation::Silu,
        })
    }
}

impl Module for Mlp {
    fn forward(&self, xs: &Tensor) -> local_inference_helpers::candle_core::Result<Tensor> {
...
struct Qwen2TextModel {
    embed_tokens: local_inference_helpers::candle_nn::Embedding,
...
impl Qwen2TextModel {
    fn new(cfg: &Qwen2TextEncoderConfig, vb: VarBuilder) -> Result<Self> {
        ...
        let embed_tokens = local_inference_helpers::candle_nn::embedding(
            cfg.vocab_size,
            cfg.hidden_size,
...
impl Qwen2TextEncoder {
    ...
    pub fn encode(
        ...
        let input_ids =
            Tensor::from_vec(tokens, (1, TEXT_WINDOW + drop_idx), &self.device)?;
        let mut mask = vec![0u8; TEXT_WINDOW + drop_idx];
        for value in &mut mask[..drop_idx + valid_len] {
            *value = 1;
        }
        let attn_mask =
            Tensor::from_vec(mask, (1, TEXT_WINDOW + drop_idx), &self.device)?;
...
mod tests {
    ...
    #[test]
    fn prompt_format_includes_system_and_user() {
        let prompt = format_qwen_image_prompt("a cat");
    ...
    #[test]
    fn system_prefix_is_subset_of_full_prompt() {
        let prefix = qwen_image_system_prefix();
        let full = format_qwen_image_prompt("test");
