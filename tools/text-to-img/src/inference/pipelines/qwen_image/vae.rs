...
use local_inference_helpers::candle_nn::{conv2d, Conv2d, Conv2dConfig, Module, VarBuilder};
...
fn load_3d_conv1x1_as_2d(
    ...
    load_3d_conv_as_2d(in_channels, out_channels, 1, 0, vb)
}
...
impl Module for QwenImageAttentionBlock2d {
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        let attn = local_inference_helpers::candle_nn::ops::softmax_last_dim(&attn)?;
        let x = attn.matmul(v)?;
...
impl QwenImageMidBlock2d {
    fn new(channels: usize, vb: VarBuilder) -> Result<Self> {
        Ok(Self {
            resnet0: QwenImageResidualBlock2d::new(
                channels,
                channels,
                vb.pp("resnets").pp("0"),
            )?,
            attention: QwenImageAttentionBlock2d::new(channels, vb.pp("attentions").pp("0"))?,
            resnet1: QwenImageResidualBlock2d::new(
                channels,
                channels,
                vb.pp("resnets").pp("1"),
            )?,
...
impl QwenImageVae {
    pub fn load(
        ...
        device: &local_inference_helpers::candle_core::Device,
        dtype: DType,
    ) -> Result<Self> {
...
mod tests {
    ...
    #[test]
    fn latent_constants_length() {
        assert_eq!(LATENTS_MEAN.len(), 16);
        assert_eq!(LATENTS_STD.len(), 16);
    }

    #[test]
    fn latent_std_all_positive() {
        for (i, &val) in LATENTS_STD.iter().enumerate() {
            assert!(val > 0.0, "LATENTS_STD[{i}] = {val} not positive");
        }
    }
}
