...
fn parse_container_xml(
    ...
    doc.descendants()
        ...
        .map(String::from)
        .ok_or_else(|| {
            utl_helpers::Error::Other("No rootfile found in container.xml".to_string())
        })
}
...
fn parse_opf(content: &str, opf_dir: &str) -> utl_helpers::Result<OpfData> {
    let doc = parse_xml(content)
        .map_err(|e| utl_helpers::Error::Other(format!("Invalid OPF: {e}")))?;
...
fn parse_ncx(content: &str) -> utl_helpers::Result<Vec<String>> {
    ...
    let doc = parse_xml(&content)
        .map_err(|e| utl_helpers::Error::Other(format!("Invalid NCX: {e}")))?;
